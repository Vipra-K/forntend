...
export default function PublicForm({
  ...
  const fetchForm = async () => {
    try {
      ...
      if (
        formData.settings?.openAt &&
        new Date(formData.settings.openAt) > now
      ) {
        setStatus("not-open");
        return;
      }
      ...
      if (
        formData.settings?.closeAt &&
        new Date(formData.settings.closeAt) < now
      ) {
        setStatus("closed");
        return;
      }
      ...
      if (
        formData.settings?.maxSubmissions &&
        formData.responseCount >= formData.settings.maxSubmissions
      ) {
  ...
  if (status === "not-open") {
    const openDate = form?.settings?.openAt
      ? new Date(form.settings.openAt)
      : null;
    return (
  ...
  if (status === "closed") {
    const closeDate = form?.settings?.closeAt
      ? new Date(form.settings.closeAt)
      : null;
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-red-50/20 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white border border-slate-100 p-10 rounded-2xl text-center shadow-lg shadow-slate-200/50">
    ...
    return (
      ...
      >
        <div className="max-w-md w-full bg-white border border-slate-100 p-10 rounded-2xl text-center shadow-lg shadow-slate-200/50">
          ...
          <p className="text-slate-500 font-medium mb-4">
            This form has reached its maximum number of submissions and is no
            longer active.
          </p>
        </div>
      </div>
        ...
        >
          ...
          {!submissionSettings?.redirectUrl ? (
            <button
              ...
              className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-all text-sm shadow-lg shadow-slate-200"
            >
            ...
            {form?.fields.map((field) => (
              ...
              >
                ...
                <div className="relative">
                  {(field.type.key === "text" ||
                    field.type.key === "email" ||
                    field.type.key === "url" ||
                    field.type.key === "phone") && (
                    <input
                      type={
                        field.type.key === "email"
                          ? "email"
                  ...
                  {field.type.key === "select" && (
                    <div className="space-y-3">
                      {field.options.map((opt) => {
                        const isSelected =
                          (values[field.id] || [])[0] === opt.value;
                        return (
                          <button
                            key={opt.id}
                            type="button"
                            onClick={() =>
                              handleInputChange(field.id, [opt.value])
                            }
                          ...
                          >
                            ...
                            <span
                              className={`text-sm tracking-tight ${isSelected ? "text-slate-900 font-bold" : "text-slate-600 font-medium"}`}
                            >
                              {opt.label}
                            </span>
                          </button>
                  ...
                  {field.type.key === "multiselect" && (
                    <div className="space-y-3">
                      {field.options.map((opt) => {
                        const isSelected = (values[field.id] || []).includes(
                          opt.value,
                        );
                        return (
                          <button
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      {/* {values[field.id] && (
                        <span
                          ...
                          style={{ color: form?.primaryColor }}  //shows rating in number near the stars
                        >
                          {values[field.id]}
                        </span>
                      )} */}
