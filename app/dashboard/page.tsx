    ...
    <div className="max-w-6xl px-8 lg:px-12 py-12">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <div className="flex items-center space-x-2 text-blue-600 mb-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.25em]">My Forms</span>
          </div>
          <h1 className="text-5xl font-black text-slate-900 tracking-tight mb-2">Dashboard</h1>
          <p className="text-slate-500 font-medium text-base">Create, manage, and track your forms in one place.</p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-72 bg-slate-50 animate-pulse rounded-2xl" />
          ))}
        </div>
      ) : forms.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 bg-gradient-to-br from-slate-50 to-blue-50/30 rounded-3xl border border-slate-100">
          ...
          <h3 className="text-2xl font-black text-slate-900 tracking-tight mb-2">No forms yet</h3>
          <p className="text-slate-500 font-medium text-center max-w-sm mb-8">
            Get started by creating your first form or choose from our professional templates.
          </p>
    ...
    >
            ...
            {showMenu && (
              <>
                ...
                >
                  ...
                  <button 
                    onClick={() => { setShowMenu(false); navigator.clipboard.writeText(`${window.location.origin}/f/${form.slug}`); toast.success('Link copied!'); }}
                    className="w-full flex items-center space-x-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors font-medium"
                  >
      ...
      <div className="flex-1 mb-5">
        ...
        <div className="flex items-center space-x-2 text-xs text-slate-400 font-medium">
          ...
          <span>{new Date(form.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="bg-slate-50 rounded-lg p-3 text-center">
          <div className="text-lg font-black text-slate-900">{form.fieldCount}</div>
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Fields</div>
        </div>
        <div className="bg-slate-50 rounded-lg p-3 text-center">
          <div className="text-lg font-black text-slate-900">v{form.version}</div>
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Version</div>
        </div>
      </div>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <h2 className="text-2xl font-black text-slate-900 mb-2">Delete Form?</h2>
              <p className="text-sm text-slate-600 mb-8 leading-relaxed">
                This will permanently delete <span className="font-bold text-slate-900">"{form.title}"</span> and all its responses. This action cannot be undone.
              </p>
